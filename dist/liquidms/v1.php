...
$router->with('/v1/servers', function () use ($router) {
    ...
    $router->respond('POST', '/[:serverid]?/update', function ($request) {
    ...
    $router->respond('POST', '/[:serverid]?/unlist', function ($request) {
        parse_str($request->body(), $info);
        $request->ip();
        $rooms = NetgameModel::changeServer(0, $request->ip(), $request->serverid, null, null);
        if ($rooms["rows"] > 0) {
            // No Response body
            return;
        } else {
...
$router->with('/v1/versions', function () use ($router) {
    $router->respond('GET', '/[:versionId]', function ($request, $response) {
        ...
        if ($import["error"] == 0) {
            ...
            foreach ($import["data"] as $ver_value) {
...
$router->with('/v1/rooms', function () use ($router) {
    ...
    $router->respond('GET', '/?', function ($request, $response) {
        ...
        if ($rooms["error"] == 0) {
            ...
            foreach ($rooms["data"] as $room_value) {
